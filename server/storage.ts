...
export interface IStorage {
  ...
  recalculateAllianceStats(): Promise<void>;
...
export type PlayerFilters = {
  ...
  sortBy?: string;
  sortOrder?: 'asc' | 'desc';
  hideHidden?: boolean; // Новое поле для фильтрации скрытых игроков
};
...
export class MemStorage implements IStorage {
  ...
  private updateAllianceStats(allianceName: string, server: string, powerDiff: number, memberDiff = 0) {
    ...
    if (existingAlliance) {
      const updatedAlliance = {
        ...existingAlliance,
        memberCount: Math.max(0, (existingAlliance.memberCount || 0) + memberDiff),
        totalPower: Math.max(0, (existingAlliance.totalPower || 0) + powerDiff)
      };
      ...
      const newAlliance: Alliance = {
        ...
        name: allianceName,
        server,
        memberCount: Math.max(1, memberDiff),
        totalPower: Math.max(0, powerDiff)
      };
  ...
  async getAllPlayers(filters: PlayerFilters = {}): Promise<GameProfile[]> {
    const { server, alliance, sortBy = "powerNow", sortOrder = "desc", hideHidden = false } = filters;
    ...
    if (alliance) {
      players = players.filter(p => p.alliance === alliance);
    }

    // Фильтруем скрытых игроков, если нужно
    if (hideHidden) {
      players = players.filter(p => !(p.hiddenPower || false));
    }
  ...
  async recalculateAllianceStats(): Promise<void> {
    for (const alliance of this.alliances.values()) {
      let totalPower = 0;
      let memberCount = 0;

      for (const profile of this.gameProfiles.values()) {
        if (profile.alliance === alliance.name && profile.server === alliance.server) {
          totalPower += profile.powerNow || 0;
          memberCount++;
        }
      }
      ...
      this.alliances.set(alliance.id, updatedAlliance);
    }
  }
}

export const storage = new MemStorage();
storage.recalculateAllianceStats().then(() => {
  console.log("Initial alliance statistics calculated successfully");
}).catch(error => {
  console.error("Error calculating initial alliance statistics:", error);
});
